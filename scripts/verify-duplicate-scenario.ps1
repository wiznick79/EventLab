...
try {
    $processes += Start-Process java -ArgumentList @(
        ...
        '--server.port=38081') -WorkingDirectory $repositoryRoot -WindowStyle Hidden -PassThru
    $processes += Start-Process java -ArgumentList @(
        '-jar', 'services/payment-service/target/payment-service-0.1.0-SNAPSHOT.jar',
        '--server.port=38082') -WorkingDirectory $repositoryRoot -WindowStyle Hidden -PassThru
    $processes += Start-Process java -ArgumentList @(
        '-jar', 'services/fulfilment-service/target/fulfilment-service-0.1.0-SNAPSHOT.jar',
        '--server.port=38083') -WorkingDirectory $repositoryRoot -WindowStyle Hidden -PassThru
    $processes += Start-Process java -ArgumentList @(
    ...
    Wait-ForHealth -Ports @(38081, 38082, 38083, 38080)
    ...
    $run = (Invoke-WebRequest -UseBasicParsing -Method Post -ContentType 'application/json' `
        -Body $request -TimeoutSec 10 http://localhost:38080/api/v1/runs).Content | ConvertFrom-Json
