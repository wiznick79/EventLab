...
try {
    ...
    $required = @('PAYMENT_PENDING', 'FULFILMENT_PENDING', 'COMPENSATION_PENDING',
        'PAYMENT_COMPENSATED', 'COMPENSATED')
    foreach ($state in $required) {
        if (-not ($timeline.state -contains $state)) { throw "Missing compensation state $state" }
    }
    if (@($timeline | Where-Object state -eq 'COMPENSATED').Count -ne 1 `
            -or $timeline.state -contains 'COMPLETED') {
