...
class ConsumerConcurrencyController {
    private final RequestFulfilmentProcessor processor;

    ConsumerConcurrencyController(RequestFulfilmentProcessor processor) { this.processor = processor; }

    @PutMapping
    Response configure(@RequestBody Request request) {
        validate(request.calls());
        processor.reconfigure(request.calls());
        return new Response("Fulfilment", processor.concurrency());
    }
